...
class Interpolate_S1(nn.Module):
    def __init__(self, ConvModule, padding, n_patch):
        ...
        self.top_left = nn.Parameter(torch.zeros(1, self.mid_conv.in_channels, 1, 1, 2, 2))
        self.top_right = nn.Parameter(torch.zeros(1, self.mid_conv.in_channels, 1, 1, 2, 2))
        self.bot_left = nn.Parameter(torch.zeros(1, self.mid_conv.in_channels, 1, 1, 2, 2))
        self.bot_right = nn.Parameter(torch.zeros(1, self.mid_conv.in_channels, 1, 1,2, 2))
