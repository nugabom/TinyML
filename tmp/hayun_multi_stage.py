...
class OutPutBuffer:
    ...
    # change for 3x3
    def loop(self, outs, cnt, index):
        for i in range(cnt ** 2):
            out = outs[f"out{i + index}"]
...
output_buffer2 = OutPutBuffer()
output_buffer3 = OutPutBuffer()
output_buffer4 = OutPutBuffer()
...
def memory_schedule(ops):
    ...
    for i, op in enumerate(ops):
        color='inference'
        if 'per_patch' in dir(op.op_node):
            print(op.op_node) 
        flag = False
        unallocated_tensors = []
        if "Store" in op.op_type:
            ...
            for idx in range(i + 1, len(ops)):
                for input_t in ops[idx].input_tensors:
                    if str(te.graph_id) == str(input_t.graph_id):
                        ...
                        end_idx = idx + 1
            if hasattr(op.op_node, 'per_patch1'):
                end_idx = start_idx
            if hasattr(op.op_node, 'per_patch2'):
                end_idx = start_idx
            if hasattr(op.op_node, 'per_patch3'):
                end_idx = start_idx
            if hasattr(op.op_node, 'per_patch4'):
                end_idx = start_idx

            if hasattr(op.op_node, "per_patch1"):
                print('per_patch1')
                stage_1_end = end_idx
            elif hasattr(op.op_node, "per_patch2"):
                print('per_patch2')
                stage_2_end = end_idx
            elif hasattr(op.op_node, "per_patch3"):
                print('per_patch3')
                stage_3_end = end_idx

            if hasattr(op.op_node, "is_start_of_normal_inference_block"):
                if getattr(op.op_node, "is_start_of_normal_inference_block"):
                    #print('kucha', op.op_node)
                    if te in op.input_tensors:
                        color='out_buf'
                        print("buffer size", out_buf//1024, out_buf2//1024, out_buf3//1024)
                        #print(stage_1_end, stage_2_end, stage_3_end)
                        start_idx = stage_3_end
                        # stage 1
                        allocator.addTensor(0, stage_2_end, out_buf, name='stage1_output_buf', type='out_buf')
                        # stage 2
                        allocator.addTensor(stage_1_end, stage_3_end, out_buf2, name='stage2_output_buf', type='out_buf')
                        # stage 3
                        #allocator.addTensor(stage_2_end, end_idx, out_buf3, name='stage3_output_buf', type='out_buf')
                        print('kucha', te.tensor.size(), te.graph_id, end_idx)
            if flag:
                pass
...
class Hconv2d_stride1(nn.Module):
    def __init__(self, ConvModule, parent):
        ...
        self.hb = 0
        if hasattr(self.mid_conv, 'per_patch1'):
            self.per_patch1 = True
        if hasattr(self.mid_conv, 'per_patch2'):
            self.per_patch2 = True
        if hasattr(self.mid_conv, 'per_patch3'):
            self.per_patch3 = True
        if hasattr(self.mid_conv, 'per_patch4'):
            self.per_patch4 = True
...
class InvertedResidual(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.use_res_connect:
            return self.conv[-1](x.clone(), self.conv[:-1](x))
        else:
            return self.conv(x)               
...
class MB(nn.Module):
    def __init__(self):
        ...
        self.conv18 = Conv2dNormActivation(112, 448, kernel_size=1, norm_layer=nn.BatchNorm2d, activation_layer=nn.ReLU6)
        self.classifier = nn.Linear(448, 1000)
        """
        setattr(self.conv4.conv[-1], 'per_patch1', True)
        setattr(self.conv5.conv[-1], 'per_patch2', True)
        setattr(self.conv6.conv[-1], 'per_patch3', True)
        setattr(self.conv7.conv[-1], 'per_patch4', True)

        self.conv4.conv[-1].per_patch1 = True
        self.conv5.conv[-1].per_patch2 = True
        self.conv6.conv[-1].per_patch3 = True
        self.conv7.conv[-1].per_patch4 = True

        #print('conv5', self.conv5)
        #print('conv6', self.conv6)
        #print('conv7', self.conv7)
        setattr(self.conv10.conv[0], "is_start_of_normal_inference_block", True)
        #self.pool = nn.AdaptiveAvgPool2d((1, 1))
    def is_hybrid(self):
    ...
    def patch_forward(self, x):
        ...
        global out_buf, out_buf2, out_buf3, out_buf4
        out_buf = np.prod(out.size())
        
        output_buffer.set_out_size(out)
        output_buffer.loop(out_dict, ph, index=0)
        #allocator.addTensor(
        ...
        stage2_ph, stage2_pw = 14,14

        out = rearrange(out, "B C (ph H) (pw W) -> (B ph pw) C H W", ph=stage2_ph, pw=stage2_pw)

        for pid2, y in enumerate(out):
            pid = pid2 + stage_2
            set_patch_id(self, pid)
            x1 = y.unsqueeze(dim=0)
            x1 = self.conv5(x1)
            #x1 = self.conv6(x1)
            #x1 = self.conv7(x1)
            out_dict[f"out{pid}"] = x1
        ...
        out = []
        for j in range(stage2_ph):
            row = []
            for i in range(stage2_pw):
                row.append(out_dict[f"out{stage_2 + stage2_ph * j + i}"])
            row = t.cat(row, dim=-1)
            out.append(row)
        out = t.cat(out, dim=-2)
        print("out2", out.size())
        out_buf2 = np.prod(out.size())
        output_buffer2.set_out_size(out)
        output_buffer2.loop(out_dict, stage2_ph, index=stage_2)
        
        stage3_ph, stage3_pw = 14,14

        out = rearrange(out, "B C (ph H) (pw W) -> (B ph pw) C H W", ph=stage3_ph, pw=stage3_pw)

        stage_3 = pid

        for pid3, y in enumerate(out):
            pid = stage_3 + pid3
            set_patch_id(self, pid)
            x1 = y.unsqueeze(dim=0)
            x1 = self.conv6(x1)
        ...
        out = []
        for j in range(stage3_ph):
            row = []
            for i in range(stage3_pw):
                row.append(out_dict[f"out{stage_3 + stage3_ph * j + i}"])
            row = t.cat(row, dim=-1)
            out.append(row)
        out = t.cat(out, dim=-2)
        print("out3", out.size())
        out_buf3 = np.prod(out.size())
        output_buffer3.set_out_size(out)
        output_buffer3.loop(out_dict, stage3_ph, index=stage_3)
        
        stage4_ph, stage4_pw = 14,14

        out = rearrange(out, "B C (ph H) (pw W) -> (B ph pw) C H W", ph=stage4_ph, pw=stage4_pw)

        stage_4 = pid

        for pid4, y in enumerate(out):
            pid = stage_4 + pid4
            set_patch_id(self, pid)
            x1 = y.unsqueeze(dim=0)
            x1 = self.conv7(x1)
        ...
        out = []
        for j in range(stage4_ph):
            row = []
            for i in range(stage4_pw):
                row.append(out_dict[f"out{stage_4 + stage4_ph * j + i}"])
            row = t.cat(row, dim=-1)
            out.append(row)
        out = t.cat(out, dim=-2)
        out_buf4 = np.prod(out.size())
        output_buffer4.set_out_size(out)
        output_buffer4.loop(out_dict, stage4_ph, index=stage_4)
        #x = self.conv7(out)
        x = self.conv11(out)
        x = self.conv12(x)
        x = self.conv13(x)
...
def allocate_SRAM(ops):
  for i, op in enumerate(ops):
      for cnt, te in enumerate(op.input_tensors):
          ...
          if t.equal(te.tensor, output_buffer2.final_out):
              output_buffer.params['output_buf_add_offset'] = allocator.getIdxAddress(te.allocator_idx)
              output_buffer.params['output_buf_add'] = "front"
          if cnt == 0:
      ...
      for cnt, te in enumerate(op.output_tensors):
          if cnt == 0:
              ...
              if hasattr(op.op_node, 'per_patch2'):
                  pid = op.params['patch_id']
                  output_buffer2.params[f"patch_out{pid}_buf_add_offset"] = allocator.getIdxAddress(te.allocator_idx)
                  output_buffer2.params[f"patch_out{pid}_buf_add"] = "front"
          if cnt == 1:
...
set_numbering(model)
#change_model(model, ph, module_to_mapping, [0, 0, 0, 0, 0])
#, 1, 1, 1])
model.patching = True
...
allocator.visualize('./lifecycles_LARGE_4442.png')
