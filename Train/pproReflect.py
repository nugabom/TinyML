...
class Mean_2px_stride1(nn.Module):
    ...
    def forward(self, x):
        ...
        pad_top = x_patches[:, :, :, :, 1:2, :].mean(dim=-2, keepdim=True)
        pad_top[:, 0, :, :, :, :] *= 0

        pad_down = x_patches[:, :, :, :, -2:-1, :].mean(dim=-2, keepdim=True)
        pad_down[:, -1, :, :, :, :] *= 0

        pad_left = x_patches[:, :, :, :, :, 1:2].mean(dim=-1, keepdim=True)
        pad_left[:, :, 0, :, :, :] *= 0

        pad_right = x_patches[:, :, :, :, :, -2:-1].mean(dim=-1, keepdim=True)
        pad_right[:, :, -1, :, :, :] *= 0
