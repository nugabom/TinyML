...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        def _forward():
            with amp_autocast():
                ...
                out_loss = loss_fn(output[5], output1[5])
            if accum_steps > 1:
                #loss /= accum_steps
            ...
            return padding0_loss, padding1_loss, padding2_loss, padding3_loss, padding4_loss, out_loss
        ...
        if has_no_sync and not need_update:
            with model.no_sync():
                padding0_loss, padding1_loss, padding2_loss, padding3_loss, padding4_loss, out_loss = _forward()
        ...
        if update_idx % args.log_interval == 0:
            ...
            if utils.is_primary(args):
                _logger.info(
                    ...
                    f'padding0_loss: {padding0_loss_m.val:#.5g} ({padding0_loss_m.avg:#.5g})  '
                    f'padding1_loss: {padding1_loss_m.val:#.5g} ({padding1_loss_m.avg:#.5g})  '
                    f'padding2_loss: {padding2_loss_m.val:#.5g} ({padding2_loss_m.avg:#.5g})  '
                    f'padding3_loss: {padding3_loss_m.val:#.5g} ({padding3_loss_m.avg:#.5g})  '
                    f'padding4_loss: {padding4_loss_m.val:#.5g} ({padding4_loss_m.avg:#.5g})  '
                    f'out_loss: {out_loss_m.val:#.5g} ({out_loss_m.avg:#.5g})  '
    ...
    return OrderedDict([
        ('loss', losses_m.avg), 
        ('padding0_loss', padding0_loss_m.avg),
        ('padding1_loss', padding1_loss_m.avg),
        ('padding2_loss', padding2_loss_m.avg),
        ('padding3_loss', padding3_loss_m.avg),
        ('padding4_loss', padding4_loss_m.avg),
        ('out_loss', out_loss_m.avg),
        ])
