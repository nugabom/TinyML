...
def padding_prediction_all(patch):
    # Border
    pad_top = patch[:, :, :3, :].mean(dim=-2, keepdim=True)
    pad_down = patch[:, :, -3:, :].mean(dim=-2, keepdim=True)
    pad_left = patch[:, :, :, :3].mean(dim=-1, keepdim=True)
    pad_right = patch[:, :, :, -3:].mean(dim=-1, keepdim=True)
...
def padding_prediction_S2_remove_topleftright(patch):
    pad_bot = patch[:, :, -3:, :].mean(dim=-2, keepdim=True)
