...
def resume_checkpoint(
        ...
        get_pre_train_model_weight: bool = False,
):
    ...
    if os.path.isfile(checkpoint_path):
        ...
        if isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
            ...
            if get_pre_train_model_weight:
                to_key = model.state_dict().keys()
                pre_key = state_dict.keys()
                assert len(to_key) == len(pre_key), 'Error Architecture difference'
                new_checkpoint = {}
                for to, pre in zip(to_key, pre_key):
                    new_checkpoint[to] = state_dict[pre]
