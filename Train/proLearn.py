...
class Mean_2px_stride1(nn.Module):
    def __init__(self, ConvModule, padding, num_patches):
        ...
        oc, ic, k, k = self.mid_conv.weight.size()
        self.inc = ic

        self.alpha = nn.Parameter(0.5 * torch.ones(self.inc))
        self.beta = nn.Parameter(0.5 * torch.ones(self.inc))
        self.gamma = nn.Parameter(torch.ones(self.inc))
    ...
    def forward(self, x):
        ...
        #pad_top = x_patches[:, :, :, :, 0, :].clone().unsqueeze(dim= -2)
        pad_top = self.alpha.view(1, 1, 1, self.inc, 1) * x_patches[:, :, :, :, 0, :].clone() + self.beta.view(1, 1, 1, self.inc, 1) * x_patches[:, :, :, :, 1, :].clone()
        pad_top = pad_top.unsqueeze(dim=-2)
        pad_top[:, 0, :, :, :, :] *= 0

        #pad_down = x_patches[:, :, :, :, -2:, :].mean(dim=-2, keepdim=True)
        #pad_down = x_patches[:, :, :, :, -1, :].clone()
        pad_down = self.alpha.view(1, 1, 1, self.inc, 1) * x_patches[:, :, :, :, -1, :].clone() + self.beta.view(1, 1, 1, self.inc, 1) * x_patches[:, :, :, :, -2, :].clone()
        pad_down = pad_down.unsqueeze(dim=-2)
        pad_down[:, -1, :, :, :, :] *= 0

        #pad_left = x_patches[:, :, :, :, :, :2].mean(dim=-1, keepdim=True)
        pad_left = self.alpha.view(1, 1, 1, self.inc, 1) * x_patches[:, :, :, :, :, 0].clone() + self.beta.view(1, 1, 1, self.inc, 1) * x_patches[:, :, :, :, :, 1].clone()
        pad_left = pad_left.unsqueeze(dim=-1)
        #pad_left = x_patches[:, :, :, :, :, 0].clone().unsqueeze(dim=-1)
        pad_left[:, :, 0, :, :, :] *= 0

        #pad_right = x_patches[:, :, :, :, :, -2:].mean(dim=-1, keepdim=True)
        pad_right = self.alpha.view(1, 1, 1, self.inc, 1) * x_patches[:, :, :, :, :, -1].clone() + self.beta.view(1, 1, 1, self.inc, 1) * x_patches[:, :, :, :, :, -2].clone()
        pad_right = pad_right.unsqueeze(dim=-1)
        #pad_right = x_patches[:, :, :, :, :, -1].clone().unsqueeze(dim=-1)
        pad_right[:, :, -1, :, :, :] *= 0

        pad_topleft = self.gamma.view(1, 1, 1, self.inc) * x_patches[:, :, :, :, 0, 0].clone()
        pad_topleft = pad_topleft.unsqueeze(dim=-1).unsqueeze(-1)
        pad_topleft[:, 0, :, :, :, :] *= 0
        pad_topleft[:, :, 0, :, :, :] *= 0

        pad_topright = self.gamma.view(1, 1, 1, self.inc) * x_patches[:, :, :, :, 0, -1].clone()
        pad_topright = pad_topright.unsqueeze(dim=-1).unsqueeze(-1)
        pad_topright[:, 0, :, :, :, :] *= 0
        pad_topright[:, :, -1, :, :, :] *= 0

        
        pad_botleft = self.gamma.view(1, 1, 1, self.inc) * x_patches[:, :, :, :, -1, 0].clone()
        pad_botleft = pad_botleft.unsqueeze(dim=-1).unsqueeze(-1)
        pad_botleft[:, -1, :, :, :, :] *= 0
        pad_botleft[:, :, 0, :, :, :] *= 0
        
        pad_botright = self.gamma.view(1, 1, 1, self.inc) * x_patches[:, :, :, :, -1, -1].clone()
        pad_botright = pad_botright.unsqueeze(dim=-1).unsqueeze(-1)
        pad_botright[:, -1, :, :, :, :] *= 0
        pad_botright[:, :, -1, :, :, :] *= 0
