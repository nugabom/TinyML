...
def padding_prediction_all(patch):
    # Border
    pad_top = patch[:, :, 0, :] + alpha * (patch[:, :, 0, :] - patch[:, :, 1, :])
    pad_top = pad_top.unsqueeze(dim=-2)

    pad_down = patch[:, :, -1, :] + alpha * (patch[:, :, -1, :] - patch[:, :, -2, :])
    pad_down = pad_down.unsqueeze(dim=-2)

    pad_left = patch[:, :, :, 0] + alpha * (patch[:, :, :, 0] - patch[:, :, :, 1])
    pad_left = pad_left.unsqueeze(dim=-1)

    pad_right = patch[:, :, :, -1] + alpha * (patch[:, :, :, -1] - patch[:, :, :, -2])
...
def padding_prediction_S2_remove_topleftright(patch):
    ...
    return torch.cat([patch, pad_down], dim=-2)
