...
def is_selection_ok(dic, samples_per_class, num_classes):
    total_count = [len(v) for (k, v) in dic.items()]
    print(total_count)
    if sum(total_count) >= samples_per_class * num_classes:
        return True

    return False

def get_calibration_dataset(dataset, dic, samples_per_class, num_classes):
    for i, (image, label) in enumerate(dataset):
        if is_selection_ok(dic, samples_per_class, num_classes):
            break
        if len(dic[label]) >= samples_per_class:
            continue
        dic[label].append(i)
...
def main():
    ...
    print("kucha experiment")
    class_indices = defaultdict(list)
    calibration_indices = get_calibration_dataset(dataset_train, class_indices, 5, args.num_classes)
    print(calibration_indices)
    calibration_dataset = torch.utils.data.Subset(dataset_train, calibration_indices)
    ...
    try:
        for tuning_layer in range(5):
            progressive = [0, 0, 0, 0, 0]
            progressive[tuning_layer] = 1
            for epoch in range(start_epoch, num_epochs):
                ...
                print(f"========================== {tuning_layer} layer: {lr_scheduler._get_lr(epoch)[0]} ==============")
                #tuning_mode(model)
                alternative_tuning(model, progressive)
                show_status(model)
                model.with_feat = None
                set_forward_lr(model, lr_scheduler._get_lr(epoch)[0])
            ...
            train_metrics = train_one_epoch(
                ...
                if saver is not None:
                    ...
                    best_metric, best_epoch = saver.save_checkpoint(epoch + num_epochs * tuning_layer, metric=save_metric)
