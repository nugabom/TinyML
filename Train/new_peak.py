...
class ConvBatchReLU(nn.Module):
    def __init__(self, Conv, Batch, use_relu, residual):
        super().__init__()
        ...
        self.num_features = Batch.num_features
        self.eps = Batch.eps
        self.running_mean = Batch.running_mean
        self.running_var = Batch.running_var
        self.batch_bias = Batch.bias
        self.batch_weight = Batch.weight

        self.use_relu = use_relu
        self.residual = residual

        self.name = f"Conv({self.in_channels}, {self.out_channels}, k={self.kernel_size}), s={self.stride})"
        self.spatial = True if self.kernel_size[0] > 1 else False

    def forward(self, x):
        x = F.conv2d(x, self.conv_weight, bias=None, stride=self.stride, padding=self.padding, groups=self.groups)
        x = F.batch_norm(x, self.running_mean, self.running_var, weight=self.batch_weight, bias=self.batch_bias, eps=self.eps, training=False)
        if self.use_relu:
            x = F.relu6(x)
        return x
...
def find_buffer_windows(stages, peak_idx):
    ...
    while input_buf_iter >= 0:
        ...
        if isinstance(stages[input_buf_iter].ops[0], ConvBatchReLU) and stages[input_buf_iter].ops[0].in_channels == 3:
            candidate = 0
...
for block in model.features:
    op_list = []
    search = [op for op in block.modules() if isinstance(op, (Add, nn.Conv2d, nn.BatchNorm2d, nn.ReLU6))]
    start_idx = 0
    for i, op in enumerate(copy.deepcopy(search)):
        if isinstance(op, (nn.Conv2d, Add)) and i != 0:
            op_list.append(search[start_idx:i])
            start_idx = i
        if i == len(search) - 1:
            op_list.append(search[start_idx: i+1])
    for subgraph in op_list:
        if len(subgraph) == 3:
            ops.append(ConvBatchReLU(subgraph[0], subgraph[1], use_relu=True, residual=getattr(block, 'use_res_connect', False)))
        elif len(subgraph) == 2:
            ops.append(ConvBatchReLU(subgraph[0], subgraph[1], use_relu=False, residual=getattr(block, 'use_res_connect', False)))
        elif len(subgraph) == 1:
            ops.append(Add())
        else:
            assert 1==0, "graph is not valid"
...
for i, op in enumerate(ops):
    op.register_forward_hook(lambda m, input, output: module_profiling(m, input, output))
    if isinstance(op, Add):
        setattr(op, 'residual', None)
        setattr(op, 'spatial', False)
        setattr(op, 'name', 'Add')
        
    setattr(op, 'layer_id', i)

def layer_wise_inference(ops, x):
    is_first_layer_in_residual_block = False
    buffering_tensor_add = None

    for op in ops:
        if not is_first_layer_in_residual_block and op.residual:
            buffering_tensor_add = x.clone()
            is_first_layer_in_residual_block = True
        if isinstance(op, ConvBatchReLU):
            x = op(x)
            if buffering_tensor_add is not None:
                setattr(op, 'residual', buffering_tensor_add.size())
            else:
                setattr(op, 'residual', None)

        else:
            #print(buffering_tensor_add.size(), x.size())
            x = op(x, buffering_tensor_add)
            is_first_layer_in_residual_block = False
            buffering_tensor_add = None

layer_wise_inference(ops, torch.randn(1, 3, 224, 224))
a="""
layer_id = 0
for block in model.features:
...
def try_fusion(stages, input_buf_idx, output_buf_idx, N=set_N):
    ...
    if isinstance(try_stages[0].ops[0], ConvBatchReLU) and try_stages[0].ops[0].in_channels == 3:
...
idx, prev_peak_mem = get_stage_with_peak_s(stages)
step = 1
prev_peak_mem = peak
while True:
    print(f'fusion{step} : {idx}, {peak_mem}')
    stages = goo(stages, idx, peak_mem)
    #show_layer_mem(stages)
    idx, peak_mem = get_stage_with_peak_s(stages)
    if prev_peak_mem <= peak_mem:
        break
    prev_peak_mem = peak_mem
