...
class Interpolate_S1(nn.Module):
    def __init__(self, ConvModule, padding, n_patch):
        ...
        self.top_left = nn.Parameter(torch.randn(1, self.mid_conv.in_channels, 1, 1, 2))
        self.top_right = nn.Parameter(torch.randn(1, self.mid_conv.in_channels, 1, 1, 2))
        self.bot_left = nn.Parameter(torch.randn(1, self.mid_conv.in_channels, 1, 1, 2))
        self.bot_right = nn.Parameter(torch.randn(1, self.mid_conv.in_channels, 1, 1, 2))
        ...
        self.top_left_m = nn.Parameter(torch.zeros(1, self.mid_conv.in_channels, 1, 1, 2))
        self.top_right_m = nn.Parameter(torch.zeros(1, self.mid_conv.in_channels, 1, 1, 2))
        self.bot_left_m = nn.Parameter(torch.zeros(1, self.mid_conv.in_channels, 1, 1, 2))
        self.bot_right_m = nn.Parameter(torch.zeros(1, self.mid_conv.in_channels, 1, 1, 2))
    ...
    def alpha_update(self, patches, x):
        ...
        target_top_left = patches[:, :, :, :, :1, 0]
        target_top_right = patches[:, :, :, :, :1, -1]
        target_bot_left = patches[:, :, :, :, -1:, 0]
        target_bot_right = patches[:, :, :, :, -1:, -1]

        diag = torch.diagonal(patch2, dim1=-1, dim2=-2)
        reverse_diag = torch.diagonal(torch.flip(patch2, [-1]), dim1=-1, dim2=-2)

        top_left = diag[:, :, :, :, :2]
        bot_right = diag[:, :, :, :, -2:]

        top_right = reverse_diag[:, :, :, :, :2]
        bot_left = reverse_diag[:, :, :, :, -2:]

        pred_top_left = (self.top_left * top_left).sum(dim=-1, keepdim=True)
        pred_top_right = (self.top_right * top_right).sum(dim=-1, keepdim=True)
        pred_bot_left = (self.bot_left * bot_left).sum(dim=-1, keepdim=True)
        pred_bot_right = (self.bot_right * bot_right).sum(dim=-1, keepdim=True)
        ...
        mse_loss_top_left = 2 * (pred_top_left - target_top_left) * top_left
        mse_loss_top_right = 2 * (pred_top_right - target_top_right) * top_right
        mse_loss_bot_left = 2 * (pred_bot_left - target_bot_left) * bot_left
        mse_loss_bot_right = 2 * (pred_bot_right - target_bot_right) * bot_right
    ...
    def predict(self, x):
        ...
        diag = torch.diagonal(patch, dim1=-1, dim2=-2)
        reverse_diag = torch.diagonal(torch.flip(patch, [-1]), dim1=-1, dim2=-2)
        ...
        pad_topleft = (self.top_left * top_left).sum(dim=-1, keepdim=True)
        pad_topleft = pad_topleft.unsqueeze(dim=-1)
        pad_topright = (self.top_right * top_right).sum(dim=-1, keepdim=True)
        pad_topright = pad_topright.unsqueeze(dim=-1)
        pad_botleft = (self.bot_left * bot_left).sum(dim=-1, keepdim=True)
        pad_botleft = pad_botleft.unsqueeze(dim=-1)
        pad_botright = (self.bot_right * bot_right).sum(dim=-1, keepdim=True)
        pad_botright = pad_botright.unsqueeze(dim=-1)
