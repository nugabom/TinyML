...
def try_fusion(stages, input_buf_idx, output_buf_idx, N=set_N):
    ...
    try_stages1 = deepcopy(stages[input_buf_idx:output_buf_idx + 1])
    try_stages = []
    for s in try_stages1:
        for op in s.ops:
            try_stages.append(Stage(op))
    #print(len(try_stages))
    print(try_stages)
    ...
    if USE_BUFFER and use_buffering:
        #fused_stage.buffer_mem = get_buffer_memory_in_fusion(fused_stage)
        #print(fused_stage.buffer_overlap)
        last_spatial = [i for i, spatial_value in enumerate(fused_stage.buffer_overlap) if spatial_value != 2]
        if last_spatial == []:
            fused_stage.buffer_mem  = 0
        else:
...
def agressive_fusion(total_stages, peak_idx, peak_mem):
    prev_stages = total_stages

    flags = True
    while flags:
        fused_stage, _ = try_fusion(prev_stages, peak_idx, peak_idx + 1, N=set_N)
        trying = concat_stages(prev_stages, [], fused_stage, peak_idx, 0, 0, peak_idx+1)

        _, try_peak = get_stage_with_peak(trying)
        if try_peak <  peak_mem:
            prev_stages = trying
            peak_mem = try_peak
            for idx, stage in enumerate(trying):
                if fused_stage is stage:
                    peak_idx = idx
        else:
            flags = False
    flags = True
    while flags:
        if peak_idx == 0:
            break
        fused_stage, _ =try_fusion(prev_stages, max(peak_idx - 1, 0), peak_idx, N=set_N)
        trying = concat_stages(prev_stages, [], fused_stage, peak_idx-1, 0, 0, peak_idx)
...
def hoo(stages, idx, peak_mem):
    if isinstance(idx, int):
        s, _ = argressive_fusion(stages, idx, peak_mem)
        return s
    temp = []
    mems = []
    for e_idx in idx:
        s, mem = agressive_fusion(stages, e_idx, peak_mem)
...
stages = hoo(stages, idx, peak_mem)
show_layer_mem(stages)
