...
class Interpolate_S1(nn.Module):
    ...
    def alpha_update(self, patches, x):
        ...
        mse_loss_top = 2 * (pred_top - target_top) * patch2[:, :, :, :, :2, :] * relu_grad(pred_top)
        mse_loss_bot = 2 * (pred_bot - target_bot) * patch2[:, :, :, :, -2:, :] * relu_grad(pred_bot)
        mse_loss_left = 2 * (pred_left - target_left) * patch2[:, :, :, :, :, :2] * relu_grad(pred_left)
        mse_loss_right = 2 * (pred_right - target_right) * patch2[:, :, :, :, :, -2:] * relu_grad(pred_right)
